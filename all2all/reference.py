...
class PyTorchAllToAll:
    ...
    def dispatch(self, dp_x: torch.Tensor, indices: torch.Tensor):
        ...
        # 嵌套列表 meta_map[r] 存储需要发送给 rank r 的所有token对应的metadata【combine 要用到】
        ...
        for t, expert_list in enumerate(indices.tolist()):      # 从indices中提取每个token选中专家的情况，t是第几个token，expert_list是每个token选中专家的list
            for k, e in enumerate(expert_list):             # 从expert_list中提取专家用于记录metadata：k代表list中的第几个专家，e代表具体的专家id
                ...
                send_counts[dst_rank] += 1                  # 这里只会根据rank统计，因为alltoallsingle要的就是rank到rank的收发量
        ...
        for i in range(total_recv):                             # total_recv：本 rank 从 other rank 接收到的所有 token 数量总和，i 代表一个 token
            ...
            local_eid = global_eid % self.num_local_experts     # 专家 本地id，在这里是写入的位置，可以看作“写指针”
            
            # 6.2 token&metadata 的存储（根据计数器，控制写入位置）
            # expert_num_tokens[local_eid]：这个本地 expert 目前已经写入了多少条 token，等价于该桶的写指针/当前长度。
            # expert_num_tokens 不如改名字叫 local_expert_recv_token_cnt
    ...
    def combine(
        ...
        # ------ 5.接收侧根据 src_token和src_k 将接收到的专家计算结果写到输出token序列中（token顺序和输入token序列保持一致）-----------
        for i in range(total_recv):             # 遍历 rank 收到的所有 token
            src_token = int(recv_meta[i, 2].item())     # 从 metadata[2] 中获取 这个token在原始输入张量中的索引位置
            src_k = int(recv_meta[i, 3].item())         # 从 metadata[3] 中获取 当前专家在token选中的topk专家中排第几？
            src_rank = int(recv_meta[i, 1].item())      # 没用上
            w = weights[src_token, src_k].to(torch.float32)                 # Gating 会给专家打分并选取 topk 专家，根据 src_token和src_k 获取到 gating 给这个token的这个专家打了多少分
            # 将打分结果（topk 权重）与 tensor（转FP32保证浮点数累加稳定性）加权输出
            # 根据 src_token 的 index，写回到它来的位置，保证 moe输出token序列内的token顺序 和输入一致；
            out_tokens[src_token] += recv_buf[i].to(torch.float32) * w      # 如果同一token有多个专家贡献，会在这个加法中累积
